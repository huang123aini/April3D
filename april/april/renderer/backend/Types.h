...
struct SamplerDescriptor {
    ...
    SamplerAddressMode s_address_mode = SamplerAddressMode::CLAMP_TO_EDGE;
    SamplerAddressMode t_address_mode = SamplerAddressMode::CLAMP_TO_EDGE;
    ...
    SamplerDescriptor(
        ...
        s_address_mode(s_address_mode), t_address_mode(t_address_mode) {}
};
...
struct UniformLocation
{
    ...
    operator bool() {
        if(shader_stage == ShaderStage::VERTEX_AND_FRAGMENT) {
            return location_[0] >= 0 && location_[1] >= 0;
        } else {
            return location_[int(shader_stage)] >=0;
        }
    }
    void reset() { location_[0] = location_[1] = -1; }
