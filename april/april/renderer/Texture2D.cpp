...
, alphatexture_(nullptr)
{
    ...
    texture_ = static_cast<backend::Texture2DBackend*>(backend::Device::getInstance()->newTexture(textureDescriptor));
}

Texture2D::~Texture2D()
{
#if CC_ENABLE_CACHEtexture__DATA
    VolatileTextureMgr::removeTexture(this);
#endif
    CC_SAFE_RELEASE_NULL(alphatexture_); // ETC1 ALPHA support.
...
{
    return alphatexture_ == nullptr ? 0 : alphatexture_->getBackendTexture();
}
...
bool Texture2D::initWithMipmaps(MipmapInfo* mipmaps, int mipmapsNum, backend::PixelFormat pixelFormat, backend::PixelFormat renderFormat, int pixelsWide, int pixelsHigh, bool preMultipliedAlpha)
{
    ...
    for (int i = 0; i < mipmapsNum; ++i)
    {
        ...
        if(texture_->getTextureFormat() != textureDescriptor.textureFormat)
            texture_->updateTextureDescriptor(textureDescriptor);

        if(info.compressed)
        {
            texture_->updateCompressedData(data, width, height, dataLen, i);
        }
        else
        {
            texture_->updateData(outData, width, height, i);
        }
...
bool Texture2D::updateWithData(void *data,int offsetX,int offsetY,int width,int height)
{
    if (texture_ && width > 0 && height > 0)
    {
        ...
        texture_->updateSubData(offsetX, offsetY, width, height, 0, textureData);
        return true;
    }
    return false;
}
...
bool Texture2D::initWithBackendTexture(backend::TextureBackend *texture, bool preMultipliedAlpha)
{
    ...
    CC_SAFE_RELEASE(texture_);
    texture_ = dynamic_cast<backend::Texture2DBackend*>(texture);
    CC_ASSERT(texture_);
    _pixelsWide = _contentSize.width = texture_->getWidth();
    _pixelsHigh = _contentSize.height = texture_->getHeight();
    _hasPremultipliedAlpha = preMultipliedAlpha;
...
void Texture2D::setAliasTexParameters()
{
    ...
    backend::SamplerDescriptor descriptor(
        ...
        (texture_->hasMipmaps()) ? backend::SamplerFilter::NEAREST_MIPMAP_NEAREST : backend::SamplerFilter::NEAREST, //minFilter
        backend::SamplerAddressMode::DONT_CARE, //sAddressMode
        backend::SamplerAddressMode::DONT_CARE//tAddressMode
    );
    texture_->updateSamplerDescriptor(descriptor);
}
...
void Texture2D::setAntiAliasTexParameters()
{
    ...
    backend::SamplerDescriptor descriptor(
        ...
        (texture_->hasMipmaps()) ? backend::SamplerFilter::LINEAR_MIPMAP_NEAREST : backend::SamplerFilter::LINEAR, //minFilter
        backend::SamplerAddressMode::DONT_CARE, //sAddressMode
        backend::SamplerAddressMode::DONT_CARE //tAddressMode
    );
    texture_->updateSamplerDescriptor(descriptor);
...
void Texture2D::setAlphaTexture(Texture2D* alphaTexture) {
    if (alphaTexture != nullptr) {
        alphaTexture->retain();
        CC_SAFE_RELEASE(alphatexture_);
        alphatexture_ = alphaTexture;
...
{
    texture_->updateSamplerDescriptor(desc);
}
...
void Texture2D::initProgram() {
    if(program_state != nullptr)
        return;
    
    auto& pipelineDescriptor = custom_command.getPipelineDescriptor();
    auto* program = backend::Program::getBuiltinProgram(backend::ProgramType::POSITIONtexture_);
    program_state = new (std::nothrow) cocos2d::backend::ProgramState(program);
    mvp_matrix_location = program_state->getUniformLocation("u_MVPMatrix");
    texture_location = program_state->getUniformLocation("u_texture");
    
    pipelineDescriptor.programState = program_state;
    
    //setup vertex layout
    auto vertexLayout = program_state->getVertexLayout();
    auto& attributes = program_state->getProgram()->getActiveAttributes();
    ...
    custom_command.setDrawType(CustomCommand::DrawType::ARRAY);
    custom_command.setPrimitiveType(CustomCommand::PrimitiveType::TRIANGLE_STRIP);
    custom_command.createVertexBuffer(4 * sizeof(float), 4, CustomCommand::BufferUsage::DYNAMIC);
    ...
    program_state->setTexture(texture_location, 0, texture_);
}

void Texture2D::drawAtPoint(const Vec2 &point, float globalZOrder) {
    float width = (float)_pixelsWide * max_s;
    float height = (float)_pixelsHigh * max_t;
...
void Texture2D::drawInRect(const Rect& rect, float globalZOrder) {
    initProgram();
    custom_command.init(globalZOrder);
    ...
    float vertexData[] = {
        rect.origin.x,                      rect.origin.y,                      0.0f,   max_t,
        rect.size.width + rect.origin.x,    rect.origin.y,                      max_s,  max_t,
        rect.origin.x,                      rect.size.height  + rect.origin.y,  0.0f,   0.0f,
        rect.size.width + rect.origin.x,    rect.size.height  + rect.origin.y,  max_s,  0.0f };
    
    program_state->setUniform(mvp_matrix_location, matrixMVP.m, sizeof(matrixMVP.m));
    custom_command.updateVertexBuffer(vertexData, sizeof(vertexData));
    Director::getInstance()->getRenderer()->addCommand(&custom_command);
}
